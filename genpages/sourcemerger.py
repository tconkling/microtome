...
class Matcher(object):
    '''Analog to java.util.regex.Matcher'''
...
class GeneratedSourceMerger(object):
    ...
    def merge (self, newly_generated, previously_generated):
        ...
        current_start = 0
        for match in matcher:
            merged.append(previously_generated[current_start:match.start()])
            ...
            if new_section is None:
                ...
                warn("Droping previously-generated section '%s' that's no longer generated by the template" % match.group(1))
        ...
        for new_section in sections:
            warn("Adding previously-missing generated section '%s' before the last non-generated text" % new_section.name)
            merged.append(new_section.contents)
    ...
    def _extract_generated_section (self, matcher, input):
        ...
        if matcher.group(2) == "DISABLED":
            return Section(name, input[start_idx:matcher.end()], True)
        ...
        return Section(name, input[start_idx:matcher.end()], False)
...
if __name__ == "__main__":
    ...
    old end
    '''
    gsm = GeneratedSourceMerger()
    print gsm.merge(new, old)

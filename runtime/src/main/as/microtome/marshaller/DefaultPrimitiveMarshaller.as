    ...
    implements PrimitiveMarshaller
{
    ...
    public function validateInt (prop :IntProp) :void {
        var min :int = prop.spec.intAnnotation(Defs.MIN_ATTR, int.MIN_VALUE);
        if (prop.value < min) {
            throw new ValidationError(prop, "value too small (" + prop.value + " < " + min + ")");
        }
        var max :int = prop.spec.intAnnotation(Defs.MAX_ATTR, int.MAX_VALUE);
        if (prop.value > max) {
    ...
    public function validateNumber (prop :NumberProp) :void {
        var min :Number = prop.spec.numberAnnotation(Defs.MIN_ATTR, Number.MIN_VALUE);
        if (prop.value < min) {
            throw new ValidationError(prop, "value too small (" + prop.value + " < " + min + ")");
        }
        var max :Number = prop.spec.numberAnnotation(Defs.MAX_ATTR, Number.MAX_VALUE);
        if (prop.value > max) {
            throw new ValidationError(prop, "value too large (" + prop.value + " > " + max + ")");
        }
    }
